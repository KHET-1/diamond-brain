...
class DiamondBrain:
    ...
    def learn(
        ...
        # Opportunistically embed the new fact (non-blocking best-effort)
        key = self._fact_key(entry)
        store = self._emb_load()
        if key not in store:
            text = f"{topic}. {fact}"
...
<div class="container">
  ...
  <div class="subtitle">Knowledge + Legal Intelligence Report &mdash; Generated {datetime.now(timezone.utc).strftime('%B %d, %Y at %H:%M UTC')}</div>
  ...
  <footer>
    Diamond Brain v3.0 &mdash; Knowledge + Legal Intelligence &mdash; Owner: Ryan Cashmoney (@Tunclon)<br/>
  ...
  a.click();
    ...
    def link_pair_start(self, port: int = 7777, timeout: int = 300) -> str:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
            ...
            # Plain TCP for LAN pairing (TLS used for sync)
            with conn:
                ...
                try:
                    ...
                    msg = self._link_recv_message(conn, temp_key)
                    if msg.get("type") != "PAIR_REQUEST":
                        self._link_send_message(conn, {
    ...
    def link_pair_connect(self, host: str, port: int, token: str) -> bool:
        ...
        try:
            with socket.create_connection((host, port), timeout=30) as conn:
                    ...
                    # Receive response
                    resp = self._link_recv_message(conn, temp_key)
    ...
    # ------------------------------------------------------------------
    # Plain TCP sync — JSON+newline over raw TCP (LAN-trusted)
    # ------------------------------------------------------------------

    @staticmethod
    def _link_send_plain(sock, msg_dict: dict) -> None:
        """Send a JSON message terminated by newline over a plain TCP socket."""
        data = json.dumps(msg_dict, ensure_ascii=False) + "\n"
        sock.sendall(data.encode("utf-8"))

    @staticmethod
    def _link_recv_plain(sock, timeout: float = 60.0) -> dict:
        """Read a JSON+newline message from a plain TCP socket.

        Reads bytes until newline is found. Raises on timeout or bad JSON.
        """
    ...
    def _link_handle_plain_client(self, conn, addr) -> None:
        """Handle a single plain TCP client connection (persistent session)."""
        import threading

        identity = self.link_identity()
        if not identity:
            self._link_send_plain(conn, {
                "type": "ERROR", "reason": "Server identity not initialized",
            })
            return

        local_fp = identity["fingerprint"]
        local_name = identity.get("display_name", "Diamond Brain")
        peer_fp = None
        peer_name = None
        ts = datetime.now(timezone.utc).strftime("%H:%M:%S")

        print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.CYAN}Connection from "
              f"{addr[0]}:{addr[1]}{_C.RESET}")

        try:
            while True:
                try:
                    msg = self._link_recv_plain(conn, timeout=300)
                except (ConnectionError, json.JSONDecodeError) as e:
                    ts = datetime.now(timezone.utc).strftime("%H:%M:%S")
                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.RED}Recv error: "
                          f"{e}{_C.RESET}")
                    break
                ...
                if msg_type == "PAIR_REQUEST":
                    ...
                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.GREEN}PAIR_REQUEST "
                          f"from {peer_name} ({peer_fp[:16]}...){_C.RESET}")
                    ...
                    if not existing:
                        peers.append({
                            ...
                            "paired_at": _now_iso(),
                            "host": addr[0],
                            "port": msg.get("port", 7777),
                            "shared_topics": [],
                            "syncs_completed": 0,
                        })
                        self._link_save_peers(peers)
                        self.learn("diamond-link",
                                   f"Paired with {peer_name} ({peer_fp[:16]}) "
                                   f"via plain TCP from {addr[0]}",
                                   confidence=95, source="diamond-link-plain")
                    else:
                        # Update host/port if changed
                        existing["host"] = addr[0]
                        existing["port"] = msg.get("port", existing.get("port", 7777))
                ...
                if msg_type == "PAIR_CONFIRM":
                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.GREEN}PAIR_CONFIRM "
                          f"from {peer_name or 'unknown'}{_C.RESET}")
                    self._link_append_custody("PAIR_CONFIRM", {
                        "peer_fingerprint": peer_fp or "",
                ...
                if msg_type == "SYNC_REQUEST":
                    req_fp = msg.get("fingerprint", peer_fp or "")
                    direction = msg.get("direction", "both")
                    topics = msg.get("topics") or None

                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.CYAN}SYNC_REQUEST "
                          f"from {peer_name or req_fp[:16]} "
                          f"(dir={direction}){_C.RESET}")
                    ...
                    for p in peers:
                        ...
                        if pfp == req_fp or (req_fp and pfp.startswith(req_fp)):
                            authorized = p
                            break

                    if not authorized or authorized.get("status") != "active":
                        self._link_send_plain(conn, {
                            "type": "ERROR",
                            "reason": "Not an authorized peer",
                        })
                        print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.RED}Rejected: "
                              f"unauthorized peer {req_fp[:16]}{_C.RESET}")
                        continue

                    auth_fp = authorized["fingerprint"]

                    # Determine topics
                    shared_topics = authorized.get("shared_topics", [])
                    sync_topics = topics
                    ...
                    if direction in ("both", "pull"):
                        try:
                            peer_msg = self._link_recv_plain(conn, timeout=120)
                            if peer_msg.get("type") == "SYNC_RESPONSE":
                                peer_snapshot = {
                                    "facts": peer_msg.get("facts", []),
                                    "citations": peer_msg.get("citations", []),
                                    "source_fingerprint": auth_fp,
                                }
                                stats_received = self._link_apply_snapshot(
                                    peer_snapshot, auth_fp, direction="pull"
                                )
                                print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.GREEN}"
                                      f"  Applied +{stats_received['facts_added']}f, "
                                      f"+{stats_received['citations_added']}c"
                                      f"{_C.RESET}")
                        except (ConnectionError, json.JSONDecodeError) as e:
                            print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.YELLOW}"
                                  f"  No peer snapshot received: {e}{_C.RESET}")

                    # Update peer stats
                    peers = self._link_load_peers()
                    for p in peers:
                        if p.get("fingerprint") == auth_fp:
                            p["last_sync"] = _now_iso()
                            p["syncs_completed"] = p.get("syncs_completed", 0) + 1
                            break
                    self._link_save_peers(peers)
                    ...
                    self._link_append_sync_log({
                        "timestamp": _now_iso(),
                        "peer_fingerprint": auth_fp,
                        "peer_name": authorized.get("display_name", "Unknown"),
                        "direction": direction,
                        "method": "plain_tcp",
                        "facts_sent": len(snapshot.get("facts", [])),
                        "facts_received": stats_received["facts_added"] + stats_received["facts_updated"],
                    ...
                    self._link_append_custody("SYNC_RECV", {
                        "peer_fingerprint": auth_fp,
                        "peer_name": authorized.get("display_name", ""),
                        "direction": direction,
                        "method": "plain_tcp",
                        "facts_received": stats_received["facts_added"] + stats_received["facts_updated"],
                        "citations_received": stats_received["citations_added"] + stats_received["citations_updated"],
                    ...
                    self._link_send_plain(conn, {
                        "type": "SYNC_DONE",
                        "stats": {
                            "facts_added": stats_received["facts_added"],
                            "facts_updated": stats_received["facts_updated"],
                            "citations_added": stats_received["citations_added"],
                            "citations_updated": stats_received["citations_updated"],
                            "conflicts": len(stats_received["conflicts"]),
                        },
                    })

                    self.learn("diamond-link",
                               f"Plain TCP sync with {authorized.get('display_name', '?')} "
                               f"({direction}): sent {len(snapshot.get('facts', []))}f, "
                               f"recv +{stats_received['facts_added']}f",
                               confidence=95, source="diamond-link-plain")

                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.GREEN}{_C.CHECK} "
                          f"Sync complete with "
                          f"{authorized.get('display_name', '?')}{_C.RESET}")
                    continue

                # --- FILE_PUSH ---
                if msg_type == "FILE_PUSH":
                    ack = self._handle_file_push(msg, addr[0])
                    self._link_send_plain(conn, ack)
                    continue

                # --- DIAMOND_EVENT ---
                if msg_type == "DIAMOND_EVENT":
                    ack = self._handle_diamond_event(msg, addr[0])
                    self._link_send_plain(conn, ack)
                    continue

                # --- NOTIFY ---
                if msg_type == "NOTIFY":
                    self._handle_notify(msg, addr[0])
                    self._link_send_plain(conn, {"type": "NOTIFY_ACK"})
                    continue

                # --- PING ---
                if msg_type == "PING":
                    self._link_send_plain(conn, {"type": "PONG"})
                    continue

                # --- DISCONNECT ---
                if msg_type == "DISCONNECT":
                    print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.YELLOW}Client "
                          f"disconnected: {peer_name or addr[0]}{_C.RESET}")
                    break

                # --- Unknown ---
                print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.YELLOW}Unknown message "
                      f"type: {msg_type}{_C.RESET}")
                self._link_send_plain(conn, {
                    "type": "ERROR",
            ...
            ts = datetime.now(timezone.utc).strftime("%H:%M:%S")
            print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.RED}Client handler "
                  f"error: {e}{_C.RESET}")
        finally:
            try:
                conn.close()
    ...
    def link_serve_plain(self, port: int = 7777,
        ...
        LAN-trusted protocol — no TLS, no HMAC. One thread per client.
        Blocks indefinitely (Ctrl+C to stop).
        """
        import socket
        import threading
        ...
        if not identity:
            raise RuntimeError(
                "Diamond Link not initialized. Run link_init() first."
            )

        local_fp = identity["fingerprint"]
        connection_log: dict[str, list[float]] = {}

        print(f"\n{_C.CYAN}{_C.BOLD}  Diamond Link Plain TCP Server{_C.RESET}")
        print(f"  {_C.WHITE}Fingerprint: {_C.DIM}{local_fp[:16]}...{_C.RESET}")
        print(f"  {_C.WHITE}Listening on port {_C.GREEN}{port}{_C.RESET}")
        print(f"  {_C.DIM}Protocol: JSON+newline over TCP (LAN-trusted){_C.RESET}")
        print(f"  {_C.DIM}Press Ctrl+C to stop.{_C.RESET}\n")

        self._link_append_custody("SERVER_START", {
            "method": "plain_tcp",
            "port": port,
        })
            ...
            try:
                while True:
                    ...
                    if len(connection_log[ip]) >= 10:
                        ts = datetime.now(timezone.utc).strftime("%H:%M:%S")
                        print(f"  {_C.DIM}[{ts}]{_C.RESET} {_C.YELLOW}"
                              f"Rate limited: {ip}{_C.RESET}")
                        conn.close()
                        continue
                    connection_log[ip].append(now)

                    t = threading.Thread(
                        target=self._link_handle_plain_client,
                        args=(conn, addr),
                        daemon=True,
                    )
                    t.start()
                ...
                print(f"\n  {_C.YELLOW}Plain TCP server stopped.{_C.RESET}")

    def link_sync_plain(self, peer_fingerprint: str,
                        topics: list[str] | None = None,
                        direction: str = "both") -> dict:
        """Initiate a plain TCP sync with an authorized peer.

        Args:
            peer_fingerprint: Full fingerprint or prefix (min 6 chars)
            topics: List of topics to sync (None = all shared)
            direction: 'both', 'push', or 'pull'

        Returns: {peer, direction, facts_sent, facts_received,
                  citations_sent, citations_received, conflicts}
        """
        import socket
        ...
        if peer.get("status") != "active":
            raise ValueError(
                f"Peer is not active: {peer.get('display_name')}"
            )
        ...
        try:
            with socket.create_connection((host, port), timeout=30) as sock:
                ...
                resp = self._link_recv_plain(sock, timeout=120)
                if resp.get("type") == "ERROR":
                    raise ValueError(
                        f"Server rejected: {resp.get('reason', '?')}"
                    )
                ...
                stats = self._link_apply_snapshot(
                    server_snapshot, peer_fp, direction="pull"
                )
                ...
                # Receive SYNC_DONE
                done_msg = self._link_recv_plain(sock, timeout=60)
                remote_stats = done_msg.get("stats", {})

                # Send DISCONNECT to cleanly close
                self._link_send_plain(sock, {"type": "DISCONNECT"})

        except (ConnectionError, socket.timeout, OSError) as e:
            raise ConnectionError(f"Plain TCP sync failed: {e}") from e
        ...
        self.learn("diamond-link",
                   f"Plain TCP sync with {peer.get('display_name', '?')} "
                   f"({direction}): sent {facts_sent}f, "
                   f"recv +{stats['facts_added']}f",
                   confidence=95, source="diamond-link-plain")
    ...
    # ------------------------------------------------------------------
    # FILE_PUSH / DIAMOND_EVENT / NOTIFY — server-side handlers
    # ------------------------------------------------------------------
    ...
    def _handle_file_push(self, msg: dict, peer_ip: str) -> dict:
        """Receive a file from peer and save it to the files inbox dir."""
        import base64
        name     = msg.get("filename", "unknown")
        data_b64 = msg.get("data", "")
        sha256   = msg.get("sha256", "")

        safe_name = os.path.basename(name).replace("..", "").strip() or "received_file"
        inbox = os.path.join(os.path.dirname(self._facts_path), "inbox")
        os.makedirs(inbox, exist_ok=True)
        dest = os.path.join(inbox, safe_name)

        try:
            raw = base64.b64decode(data_b64)
        except Exception as e:
            print(f"{_C.RED}✗ FILE_PUSH decode error: {e}{_C.RESET}")
            return {"type": "FILE_ACK", "ok": False, "error": str(e)}

        actual = hashlib.sha256(raw).hexdigest()
        if sha256 and sha256 != actual:
            print(f"{_C.RED}✗ FILE_PUSH hash mismatch from {peer_ip}{_C.RESET}")
            return {"type": "FILE_ACK", "ok": False, "error": "sha256 mismatch"}

        with open(dest, "wb") as f:
            f.write(raw)

        size_kb = len(raw) / 1024
        print(f"{_C.GREEN}✓ FILE_PUSH received: {safe_name} ({size_kb:.1f} KB) "
              f"sha256:{actual[:16]}… from {peer_ip}{_C.RESET}")

        if safe_name.endswith((".json", ".zkproof", ".cert.json")):
            try:
                payload = json.loads(raw.decode("utf-8"))
                fact_text = (f"FILE_PUSH received {safe_name} from {peer_ip}: "
                             f"{json.dumps(payload)[:300]}")
                self.learn("file_inbox", fact_text, 85, f"diamond_link:{peer_ip}", False)
            except Exception:
                pass

        return {"type": "FILE_ACK", "ok": True, "filename": safe_name,
                "sha256": actual, "size": len(raw)}

    def _handle_diamond_event(self, msg: dict, peer_ip: str) -> dict:
        """Receive a Diamond Protocol event envelope and ingest it as a brain fact."""
        event   = msg.get("event", "UNKNOWN_EVENT")
        tool    = msg.get("tool", "unknown")
        case    = msg.get("case_ref", "")
        payload = msg.get("payload", {})
        ts      = msg.get("timestamp", time.strftime("%Y-%m-%dT%H:%M:%SZ"))

        env_hash = msg.get("envelope_hash", "")
        if env_hash:
            check = dict(msg)
            check["envelope_hash"] = ""
            actual = hashlib.sha256(json.dumps(check, sort_keys=True).encode()).hexdigest()
            if actual != env_hash:
                print(f"{_C.YELLOW}  ⚠ DIAMOND_EVENT envelope hash mismatch "
                      f"(tampered?){_C.RESET}")

        fact_text = (f"DIAMOND_EVENT [{event}] from {tool} at {ts}"
                     + (f" | case={case}" if case else "")
                     + f" | payload={json.dumps(payload)[:200]}")
        self.learn("diamond_events", fact_text, 90, f"diamond_link:{peer_ip}", False)

        print(f"{_C.GREEN}✓ DIAMOND_EVENT: [{event}] from {tool} ({peer_ip}){_C.RESET}")
        return {"type": "EVENT_ACK", "ok": True, "event": event}

    def _handle_notify(self, msg: dict, peer_ip: str) -> None:
        """Display a notification from a peer."""
        title = msg.get("title", "DiamondLink")
        body  = msg.get("body", "")
        ...
        print(f"\n{color}╔═══ NOTIFY [{level}] from {peer_ip} ═══{_C.RESET}")
        print(f"{color}║  {title}{_C.RESET}")
        if body:
            print(f"{color}║  {body}{_C.RESET}")
        print(f"{color}╚{'═' * 40}{_C.RESET}\n")
        self.learn("notifications",
                   f"NOTIFY [{level}] from {peer_ip}: {title} — {body}",
                   70, f"diamond_link:{peer_ip}", False)

    # ------------------------------------------------------------------
    # FILE_PUSH / DIAMOND_EVENT / NOTIFY — client-side send methods
    # ------------------------------------------------------------------

    def link_send_file(self, remote_ip: str, filepath: str, port: int = 7778,
                       timeout: int = 30) -> dict:
        """Push a file to a peer over DiamondLink.

        Args:
            remote_ip:  Peer IP, or "host:port" string
            filepath:   Local file path to send
            port:       DiamondLink TCP port (default 7778)
            timeout:    Socket timeout in seconds

        Returns: {"ok": bool, "sha256": str, "size": int, "filename": str}
        """
        ...
        msg = {
            ...
            "sha256":   sha256,
            "size":     len(raw),
            "meta":     {"sent_at": time.strftime("%Y-%m-%dT%H:%M:%SZ")},
        }

        print(f"{_C.CYAN}→ Sending {filename} ({size_kb:.1f} KB) to "
              f"{remote_ip}:{port}…{_C.RESET}")
        try:
            ...
            if ack.get("ok"):
                print(f"{_C.GREEN}✓ {filename} delivered — sha256:{sha256[:16]}…{_C.RESET}")
            else:
                print(f"{_C.RED}✗ Peer rejected: {ack.get('error')}{_C.RESET}")
            return ack
        except Exception as e:
            print(f"{_C.RED}✗ link_send_file error: {e}{_C.RESET}")
            return {"ok": False, "error": str(e)}

    def link_send_event(self, remote_ip: str, event: str, tool: str = "diamond_brain",
                        payload: dict = None, case_ref: str = "",
                        port: int = 7778, timeout: int = 10) -> dict:
        """Send a Diamond Protocol event envelope to a peer.

        Args:
            remote_ip:  Peer IP, or "host:port" string
            event:      Event name e.g. "ZK_PROOF_READY", "DRILL_DONE"
            tool:       Tool emitting the event
            payload:    Tool-specific payload dict
            case_ref:   Case identifier e.g. "QC-25-32157"

        Returns: {"ok": bool, "event": str}
        """
        import socket as _sock
        if ":" in remote_ip:
            parts = remote_ip.rsplit(":", 1)
        ...
        envelope = {
            ...
            "operator":        identity.get("display_name", "DiamondBrain"),
            "machine":         identity.get("host", ""),
            "case_ref":        case_ref,
            "event":           event,
            "payload":         payload,
            "envelope_hash":   "",
        ...
        envelope["envelope_hash"] = hashlib.sha256(
            json.dumps(envelope, sort_keys=True).encode()).hexdigest()

        print(f"{_C.CYAN}→ DIAMOND_EVENT [{event}] → {remote_ip}:{port}{_C.RESET}")
        try:
            s = _sock.socket(_sock.AF_INET, _sock.SOCK_STREAM)
            s.settimeout(timeout)
            s.connect((remote_ip, port))
            ...
            ack = json.loads(ack_raw.split(b"\n")[0])
            if ack.get("ok"):
                print(f"{_C.GREEN}✓ Event [{event}] acknowledged{_C.RESET}")
            return ack
        except Exception as e:
            print(f"{_C.RED}✗ link_send_event error: {e}{_C.RESET}")
            return {"ok": False, "error": str(e)}

    def link_notify(self, remote_ip: str, title: str, body: str = "",
                    level: str = "info", port: int = 7778, timeout: int = 10) -> bool:
        """Send a notification to a peer's DiamondLink server.
        ...
        Returns: True if peer acknowledged
        """
        import socket as _sock
        if ":" in remote_ip:
            parts = remote_ip.rsplit(":", 1)
        ...
        msg = {
            ...
            "body":  body,
            "level": level,
            "from":  identity.get("display_name", "DiamondBrain"),
        ...
        try:
            ...
            s.close()
            ack = json.loads(ack_raw.split(b"\n")[0])
            print(f"{_C.GREEN}✓ NOTIFY sent to {remote_ip}{_C.RESET}")
            return ack.get("type") == "NOTIFY_ACK"
        except Exception as e:
            print(f"{_C.RED}✗ link_notify error: {e}{_C.RESET}")
            return False
    ...
    def _emb_load(self) -> dict:
        """Load {fact_key: vector} index from disk."""
        p = self._emb_path()
        if p.exists():
            try:
    ...
    def _emb_save(self, store: dict) -> None:
        p = self._emb_path()
    ...
    def _fact_key(f: dict) -> str:
        """Deterministic key for a fact dict."""
        return f"{f.get('topic', '')}::{f.get('fact', '')[:80]}"

    def embed_facts(self, max_facts: int = 500,
                    force: bool = False) -> dict:
        """Embed all facts that don't have a stored vector yet.

        Calls LM Studio nomic-embed-text for each un-embedded fact.
    ...
    def hybrid_search(self, query: str, top_k: int = 10,
        ...
        """Combined keyword + graph + semantic search.

        Weights (when semantic available): keyword=0.4, graph=0.3, semantic=0.3
        Weights (semantic unavailable):    keyword=0.6, graph=0.4
        ...
        kw_w = 0.4 if has_semantic else 0.6
        gr_w = 0.3 if has_semantic else 0.4
        sm_w = 0.3 if has_semantic else 0.0
        ...
        for i, f in enumerate(keyword_results):
            ...
            scored[key] = scored.get(key, 0) + kw_score * kw_w
            fact_map[key] = f
        ...
        for nid, node in nodes.items():
            if query_lower in nid.lower() or query_lower in str(
                ...
                key = f"graph:{nid}"
                scored[key] = scored.get(key, 0) + graph_score * gr_w

        # Semantic scoring
        for i, f in enumerate(semantic_results):
            key = f"{f.get('topic', '')}/{f.get('fact', '')[:50]}"
            sm_score = f.get("_semantic_score", 0.0)
            scored[key] = scored.get(key, 0) + sm_score * sm_w
            fact_map.setdefault(key, f)
    ...
    # ==================================================================
    #  LM STUDIO — Auto-Categorize, Query Expand, Fact Dedup
    # ==================================================================

    def auto_categorize(self, text: str,
                        existing_topics: list[str] | None = None) -> str | None:
        """Suggest the best topic category for raw text using the fast model.

        Returns a topic string (e.g. "fraud", "witness_credibility") or None
        if LM Studio is unavailable.  Useful before calling learn() with an
        unknown or auto-detected topic.
        """
        topics_hint = ""
        if existing_topics:
            sample = existing_topics[:20]
            topics_hint = (f"\nExisting topics (pick one if it fits, or "
                           f"invent a short new one): {', '.join(sample)}")

        messages = [
            {"role": "system", "content": (
                "You are a knowledge categorization assistant. "
                "Return ONLY a single lowercase snake_case topic word or "
                "short phrase (max 3 words, no explanation)."
            )},
            {"role": "user", "content": (
                f"Categorize this fact into a topic:{topics_hint}\n\n"
                f"FACT: {text[:500]}"
            )},
        ]
        result = self._cortex_chat(messages, temperature=0.1, max_tokens=20,
                                   model=self._LM_FAST_MODEL)
        if result:
            # Sanitize — keep only safe characters
            clean = result.strip().lower().replace(" ", "_")
            clean = "".join(c for c in clean if c.isalnum() or c == "_")
            return clean[:50] if clean else None
        return None

    def query_expand(self, query: str, n: int = 4) -> list[str]:
        """Generate semantically related search terms for a query.

        Uses the fast model to produce alternative phrasings.  Returns a list
        of expanded terms including the original.  Falls back to [query] on
        LM Studio error.
        """
        messages = [
            {"role": "user", "content": (
                f"Generate {n} alternative search phrases for this query "
    ...
    def search_expanded(self, query: str, top_k: int = 10) -> list[dict]:
        """hybrid_search with LLM-expanded query terms merged and re-ranked.

        Each expansion term contributes to the scored pool, then results are
        de-duplicated and ranked by best combined score.
        """
        terms = self.query_expand(query, n=3)
        scored: dict[str, tuple[float, dict]] = {}

        for i, term in enumerate(terms):
            # Decay weight: primary term=1.0, expansions decay slightly
            weight = 1.0 - i * 0.1
            for f in self.hybrid_search(term, top_k=top_k):
                key = self._fact_key(f)
                existing = scored.get(key, (0.0, f))
                merged_score = existing[0] + f.get("_hybrid_score", 0.0) * weight
                scored[key] = (merged_score, f)

        ranked = sorted(scored.values(), key=lambda x: x[0], reverse=True)
        results = []
        for score, f in ranked[:top_k]:
            f_copy = dict(f)
            f_copy["_expanded_score"] = round(score, 4)
            results.append(f_copy)
        return results
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--link-serve-plain":
        ...
        if "--port" in sys.argv:
            idx = sys.argv.index("--port")
            if idx + 1 < len(sys.argv):
                lsp_port = int(sys.argv[idx + 1])
        brain.link_serve_plain(port=lsp_port)
        sys.exit(0)

    # CLI: --link-sync-plain <fp_prefix> [--topics t1,t2] [--direction both]
    if len(sys.argv) >= 3 and sys.argv[1] == "--link-sync-plain":
        lsp_fp = sys.argv[2]
        lsp_topics = None
        lsp_dir = "both"
        ...
        if "--topics" in sys.argv:
            idx = sys.argv.index("--topics")
            if idx + 1 < len(sys.argv):
                lsp_topics = [t.strip() for t in sys.argv[idx + 1].split(",")]
        if "--direction" in sys.argv:
            idx = sys.argv.index("--direction")
            if idx + 1 < len(sys.argv):
                lsp_dir = sys.argv[idx + 1]

        try:
            result = brain.link_sync_plain(lsp_fp, topics=lsp_topics,
                                           direction=lsp_dir)
            print(f"\n{_section_header('Plain TCP Sync Complete', _C.CHECK, _C.GREEN)}")
            print(f"    {_C.WHITE}Peer       :{_C.RESET}  {result['peer']}")
            print(f"    {_C.WHITE}Direction  :{_C.RESET}  {result['direction']}")
            print(f"    {_C.WHITE}Facts sent :{_C.RESET}  {_C.CYAN}{result['facts_sent']}{_C.RESET}")
            print(f"    {_C.WHITE}Facts recv :{_C.RESET}  {_C.GREEN}{result['facts_received']}{_C.RESET}")
            print(f"    {_C.WHITE}Cites sent :{_C.RESET}  {_C.CYAN}{result['citations_sent']}{_C.RESET}")
    ...
    # CLI: --embed-facts [--force]
    if len(sys.argv) >= 2 and sys.argv[1] == "--embed-facts":
        force = "--force" in sys.argv
        print(f"\n{_section_header('Embedding Facts', _C.DIAMOND, _C.CYAN)}")
        result = brain.embed_facts(force=force)
        print(f"    {_C.GREEN}Embedded : {result['embedded']}{_C.RESET}")
        print(f"    {_C.DIM}Skipped  : {result['skipped']}{_C.RESET}")
        if result["failed"]:
            print(f"    {_C.YELLOW}Failed   : {result['failed']} "
                  f"(LM Studio unavailable?){_C.RESET}")
        print()
        sys.exit(0)

    # CLI: --semantic-search <query>
    if len(sys.argv) >= 3 and sys.argv[1] == "--semantic-search":
        sq = " ".join(sys.argv[2:])
        results = brain.semantic_search(sq, top_k=10)
        print(f"\n{_section_header(f'Semantic Search: {sq}', _C.DIAMOND, _C.CYAN)}")
        if not results:
            print(f"    {_C.YELLOW}No results — run --embed-facts first or "
                  f"check LM Studio is running.{_C.RESET}\n")
        else:
            for i, f in enumerate(results, 1):
                score = f.get("_semantic_score", 0)
                print(f"    {_C.BOLD}{i}.{_C.RESET} "
                      f"[{f.get('topic', '?')}] "
                      f"{_C.DIM}sim={score:.3f}{_C.RESET}")
                print(f"       {f.get('fact', '')[:120]}")
        print()
        sys.exit(0)

    # CLI: --auto-categorize <text>
    if len(sys.argv) >= 3 and sys.argv[1] == "--auto-categorize":
        ac_text = " ".join(sys.argv[2:])
        facts = brain._load(brain._facts_path)
        topics = list({f.get("topic", "") for f in facts if f.get("topic")})
        topic = brain.auto_categorize(ac_text, existing_topics=topics)
        print(f"\n{_section_header('Auto-Categorize', _C.DIAMOND, _C.CYAN)}")
        if topic:
            print(f"    {_C.GREEN}Suggested topic: {_C.BOLD}{topic}{_C.RESET}")
        else:
            print(f"    {_C.YELLOW}LM Studio unavailable — cannot categorize.{_C.RESET}")
        print()
        sys.exit(0)

    # CLI: --query-expand <query>
    if len(sys.argv) >= 3 and sys.argv[1] == "--query-expand":
        qe_query = " ".join(sys.argv[2:])
        terms = brain.query_expand(qe_query, n=5)
        print(f"\n{_section_header('Query Expansion', _C.DIAMOND, _C.CYAN)}")
        for t in terms:
            marker = f"{_C.BOLD}[original]{_C.RESET}" if t == qe_query else ""
            print(f"    • {t} {marker}")
        print()
        sys.exit(0)

    # CLI: --search-expanded <query>
    if len(sys.argv) >= 3 and sys.argv[1] == "--search-expanded":
        se_query = " ".join(sys.argv[2:])
        results = brain.search_expanded(se_query, top_k=10)
        print(f"\n{_section_header(f'Expanded Search: {se_query}', _C.DIAMOND, _C.CYAN)}")
        for i, f in enumerate(results, 1):
            score = f.get("_expanded_score", 0)
            print(f"    {_C.BOLD}{i}.{_C.RESET} "
                  f"[{f.get('topic', '?')}] "
                  f"{_C.DIM}score={score:.3f}{_C.RESET}")
            print(f"       {f.get('fact', '')[:120]}")
        print()
        sys.exit(0)
