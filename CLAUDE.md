...
- **Tests:** 213 tests across 31 tiers (`.github/workflows/diamond-ci.yml`)
- **Public methods:** 116 (all in `brain/diamond_brain.py`)
...
diamond-brain/
  brain/
    ...
    diamond_brain.py        <- Core brain module (ALL logic lives here — ~9600 lines)
  ...
  tests/
    __init__.py
    test_diamond_brain.py   <- 213 tests across 31 tiers
  .github/
    workflows/
...
python -m unittest tests.test_diamond_brain  # 213 tests
```
...
[ ] 213 tests pass (python -m unittest tests.test_diamond_brain)
...
2. `python -m unittest tests.test_diamond_brain` — all 213 tests pass
...
| Witness Credibility Scoring | Done | `source_register`, `source_credibility`, `source_adjust_credibility`, `source_list`, `source_weighted_confidence`, `source_credibility_trend` |
| Timeline Anomaly Detection | Done | `temporal_detect_anomalies`, `temporal_anomaly_summary` |
| Merkle DAG for Custody | Done | `merkle_build`, `merkle_prove`, `merkle_verify_proof`, `merkle_status` |
| CRDT Sync | Done | `crdt_upgrade_facts`, `crdt_merge_snapshot`, `crdt_tombstone`, `crdt_status`, `crdt_debug_hlc` |
| Case/UCO Ontology Export | Done | `export_case_uco`, `case_validate_export` |
| Hybrid Search | Done | `hybrid_search` |
| Homomorphic Confidence | Done | `hc_initiate`, `hc_receive_commitment`, `hc_reveal`, `hc_receive_reveal`, `hc_aggregate`, `hc_status` |
| Neural Cortex | Done | `cortex_ask`, `cortex_summarize`, `cortex_hypothesize`, `cortex_cross_examine`, `cortex_case_brief`, `cortex_timeline_narrative`, `cortex_status` |
| The Third Eye | Done | `third_eye_scan`, `third_eye_summary`, `third_eye_suppress`, `third_eye_watch`, `third_eye_status` |
| Diamond Quarantine | Done | `quarantine_list`, `quarantine_stats`, `quarantine_preview`, `quarantine_restore`, `quarantine_purge`, `quarantine_status` |
